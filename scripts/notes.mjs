...
function replyHtml(r, note) {
  const who = { answer: 'TA', deeper: 'TA · thought about it', check: 'TA · on what you wrote', widget: 'TA · built', error: 'TA' }[r.kind] || r.kind;
  ...
  if (r.kind === 'you') return `<div class="reply you"><p class="who"><span class="avatar">K</span><b>Kaushik</b> · ${esc(when(r.at))}</p><p class="kb">${esc(r.text)}</p></div>`;
...
.reply.check { border-left-color:var(--gold); } .reply.you { border-left-color:var(--gold-soft); } .reply.overruled .body { color:var(--muted); }
