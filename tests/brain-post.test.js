...
test('brain.mjs refuses a session with nothing starred, then writes a post that passes the brain\'s check', { skip: !can && 'no ~/Code/brain on this machine' }, () => {
  ...
  const run = (file) => { try { return { code: 0, out: execFileSync('node', [join(root, 'scripts/brain.mjs'), '7xTGNNLPyMI', '--from', file], { env: { ...process.env, BRAIN_DIR: dir, TA_MOCK: '1' }, stdio: 'pipe' }).toString() }; } catch (e) { return { code: e.status, out: e.stderr.toString() }; } };
  const r1 = run(plain);
